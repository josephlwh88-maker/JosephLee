...
if uploaded_file is not None:
    ...
    if len(all_active_groups) >= 2:
        ...
        # =========================================================================
            ...
            # Group by player total overall spend within 14 days to cap outliers consistently
            ...
            if cap_percentile < 100.0:
                ...
                # Apply capping multiplier scaling to prevent raw/capped disparity
            ...
            if "Cumulative" in cohort_mode:
                ...
                # Merge back to all players so zero-spend users are included in cohort averages
                ...
                # Forward fill cumulative spend across days for inactive days
                ...
                # Non-Cumulative exact day behavior
                full_grid = pd.MultiIndex.from_product(
                    [all_players[col_player].unique(), range(15)], 
                    names=[col_player, 'player_age']
                ).to_frame().reset_index(drop=True)
